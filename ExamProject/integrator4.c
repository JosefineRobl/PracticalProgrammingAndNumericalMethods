...
// Variable transformation for Clenshaw-Curtis
static double fClenshawCurtis(double f(double), double t){
 ...
 */
double recursiveIntegrate( double f(double),double a, double b, double delta, double epsilon, double f2, double f3, int nrec, int variableTransformationFormula){//wrapper calls F(f,t)
	...
	if (error < tolerance) {
		...
		double Q1 = recursiveIntegrate(f, a, (a+b)/2, delta/sqrt(2), epsilon, f1, f2, nrec + 1, variableTransformationFormula);
		double Q2 = recursiveIntegrate(f, (a+b)/2, b, delta/sqrt(2), epsilon, f3, f4, nrec + 1, variableTransformationFormula);
		return Q1 + Q2;
	}
}
...
double integrate(double f(double), double a, double b, double delta, double epsilon, int variableTransformationFormula){
	...
	int nrec = 0;
	// Begin recursion
	return recursiveIntegrate(f, a, b, 2*delta, 2*epsilon, f2, f3, nrec, variableTransformationFormula);
}

/*
 * Integrates a function using the Clenshaw-Curtis transformation of the integral.
 * 
 * f: Function to be integrated. Taks a double as input and returns a double (the function value at that point).
 * a: Double containing the lower integration limit.
 * b: Double containing the upper integration limit.
 ...
 * epsilon: Double containing the relative accuracy goal.
 * 
 * returns a double containing the integrated value of the function in the integration limit using the Clenshaw-Curtis transformation.
 */
double ClenshawCurtisIntegrate(double f(double), double a, double b, double delta, double epsilon){
	return integrate(f, a, b, delta, epsilon, 4);
}	

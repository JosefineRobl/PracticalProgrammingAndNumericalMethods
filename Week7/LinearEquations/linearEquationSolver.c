...
void gramSchmidtDecomposition(gsl_matrix* A, gsl_matrix* R){
	...
	int m = A->size2;
	// Defining a pointer to a double for later use for the dot product value
	double* dotProduct;
...
void print_matrix(gsl_matrix* R){
	for (int i = 0; i < R->size1; i++) {
		for (int j = 0; j < R->size2; j++) {
			printf("%f      ", gsl_matrix_get(R, i, j));
		}
		printf("\n");
	}
}
